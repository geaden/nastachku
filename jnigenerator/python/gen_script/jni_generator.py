...
def _GetUnusedParamsInDeclaration(native, params_in_declaration):
    """Returns lines of unused params in params declarations.

    Args:
      native: the native dictionary describing the method.
      params_in_declaration: list of params to be passed in declaration.

    Returns:
      A string containing unused params.
    """
    stub_params = _GetParamsInStubList(native)
    unused = []
    for param in stub_params:
        param_name = param.split(' ')[1]
        if all(param_name not in pid for pid in params_in_declaration):
            unused += [param_name]
    return '\n  '.join(['Y_UNUSED({});'.format(p) for p in unused])
...
def _GetParamsInStubList(native):
    """Returns a list of the params for the stub declaration.

    Args:
      native: the native dictionary describing the method.

    Returns:
      A list containing the params.
    """
...
def GetParamsInStub(native):
    ...
    Returns:
      A string containing the params.
    """
    params = _GetParamsInStubList(native)
    return ',\n    '.join(params)
...
inline jclass ${JAVA_CLASS}_clazz(JNIEnv* env) {
    return env->FindClass(kClassPath_${JAVA_CLASS});
}
#endif
"""
        if declare_only:
...
""")
    ...
    def GetNativeStub(self, native):
        ...
        values = {
            ...
            'UNUSED_PARAMS': _GetUnusedParamsInDeclaration(native, params_in_declaration),
            'POST_CALL': post_call,
